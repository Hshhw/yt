...
<?php
$token = "5239892120:AAFcL6FnIhaH4nMglbBsbBiqyceWIQjvalo";
...
$xx = "https://mahmoudm50.xyz/7oda/dow.php?token=$token&chat=$chat_id&text=$text&rep=$rep&type=mp3";
$ch = curl_init($xx);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
...
file_put_contents("$chat_id","");
file_get_contents("https://api.telegram.org/bot$token/sendAnimation?chat_id=$chat_id&animation=https://t.me/youtube7odabot/7951&reply_to_message_id=$rep");
$ytt = "https://mahmoudm50.xyz/7oda/dow.php?token=$token&chat=$chat_id&text=$text&rep=$rep&type=mp4";
$ch = curl_init($ytt);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_HEADER, 0);
$data = curl_exec($ch);
...
$xt = "https://api.medooo.ml/leomedo/voiceRecognise?token=$token&chat_id=$chat_id&file_id=$idd&msg_id=$rep";
$ch = curl_init($xt);
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_HEADER, 0);
$data = curl_exec($ch);
